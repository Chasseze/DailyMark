...
- **Privacy Notice and Terms of Use** — two public pages at `/privacy` and `/terms`, linked from Settings and from under the sign-in form so they can be read before an account exists. They sit outside the auth guard: a link to them must never bounce to a sign-in screen. The privacy notice describes the app as actually built — every table and bucket that holds your data, what is kept on the device, the Supabase and Vercel dependencies, and the fact that browser dictation may send audio to the browser vendor. The operator is CharlesEZE Group, contact is hello@chasseze.com, and the Terms are governed by the law of the Federal Republic of Nigeria — one governing law, because a contract cannot name two, with users' mandatory local rights (including US state law) expressly preserved. All three live in `src/lib/legal.ts` so the two documents can never disagree

- **"Move to" is the same picker on a phone as on a desktop** — it was a native `<select>`, which a phone renders as an OS picker sheet far larger than the compact list the same markup gives on a desktop; the optgroups that separated the notebooks from Trash made that sheet taller still. It is a menu the app draws itself now, measured at an identical 208×209 on both, matching the templates and mood pickers. Notebooks carry their own colour, and Trash sits below a rule in the danger colour rather than under an optgroup label. The shared panel was lifted out of the templates picker rather than copied, so the two cannot drift
